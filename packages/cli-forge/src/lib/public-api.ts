...
export interface CLI<TArgs extends ParsedArgs = ParsedArgs> {
   ...
   */
  getHandler(): ((args: TArgs) => any) | undefined;

  /**
   * Returns child commands registered on this CLI instance.
   * Provides runtime access to sibling commands for composition.
   */
  getChildCommands(): Record<string, CLI<any>>;
}
...
export interface CLIHandlerContext {
   ...
   * Throws an error if no parent command is available (e.g., at root level).
   */
...
> {
   ...
   * @param context Optional context for accessing parent command and siblings.
   */
  builder?: (parser: CLI<TInitial>, context?: CLIHandlerContext) => CLI<TArgs>;
