...
- **Rich option types** — Native `string`, `number`, `boolean`, `array`, and `object` with automatic coercion ([example](/examples/object-arguments)). Commander treats values as strings by default; numbers require [custom processing functions](https://github.com/tj/commander.js#custom-option-processing).
